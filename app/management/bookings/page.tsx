      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white border border-cream-200 p-8 max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto">
            <button onClick={() => setShowModal(false)} className="absolute top-4 right-4 text-charcoal-700/30 hover:text-charcoal-900 transition-colors">
              <X size={16} />
            </button>
                ...
                <select value={form.roomId} onChange={e => update('roomId', e.target.value)}
                  ...
                  {rooms.map(r => (
                    <option key={r.id} value={r.id} disabled={r.status === 'booked'}>
                      {r.name} — ₱{r.price_per_night.toLocaleString()}/night {r.status === 'booked' ? '(Booked)' : ''}
                    </option>
                  ))}
                </select>
              </div>
