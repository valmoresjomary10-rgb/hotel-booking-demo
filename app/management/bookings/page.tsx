      ...
      <div className="flex gap-1 mb-4 flex-wrap">
        {filters.map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`font-accent text-[9px] uppercase tracking-widest px-4 py-2 transition-colors capitalize ${
              filter === f ? 'bg-charcoal-900 text-cream-50' : 'bg-white border border-cream-200 text-charcoal-700/50 hover:text-charcoal-900'
            }`}>
            {f} {f !== 'all' && `(${counts[f]})`}
          </button>
        ))}
      ...
      <div className="bg-white border border-cream-200 overflow-x-auto">
        {loading ? (
          <div className="p-12 text-center">
            <p className="font-body text-sm text-charcoal-700/40">Loading bookings...</p>
          </div>
        ) : (
            ...
            <tbody>
              ...
              {filtered.map(b => (
                <tr key={b.id} className="border-b border-cream-200 last:border-0 hover:bg-cream-50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-body text-sm text-charcoal-900">₱{b.total_price.toLocaleString()}</td>
                  <td className="px-6 py-4">
                    <span className={`font-accent text-[9px] uppercase tracking-widest px-2 py-1 ${statusColor(b.status)}`}>
                      {b.status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`font-accent text-[9px] uppercase tracking-widest px-2 py-1 ${paymentColor(b.payment_status)}`}>
                      {b.payment_status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <Link href={`/management/bookings/${b.id}`}
                      className="font-accent text-[9px] uppercase tracking-widest text-gold-500 hover:text-gold-600 transition-colors">
                      View
                    </Link>
                  </td>
                </tr>
