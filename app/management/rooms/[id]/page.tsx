...
import { Room, BedType, RoomStatus } from '@/types/room'
...
export default function EditRoomPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const rooms: Room[] = stored ? JSON.parse(stored) : initialRooms
  ...
  const addImage = () => {
    ...
    set('images', [...current, newImageUrl.trim()])
    setNewImageUrl('')
  }

  const removeImage = (index: number) => {
    const current = (form?.images ?? []) as string[]
    set('images', current.filter((_, i) => i !== index))
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const stored = localStorage.getItem('lum_rooms')
    const rooms: Room[] = stored ? JSON.parse(stored) : initialRooms
    const updated = rooms.map(r => r.id === id ? { ...r, ...form, updatedAt: new Date().toISOString() } : r)
        ...
        <div className="bg-white border border-cream-200 p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.bedType ?? 'queen'} onChange={e => set('bedType', e.target.value)}
                className="w-full border border-cream-200 px-3 py-2 font-body text-sm focus:outline-none focus:border-gold-400/50">
        ...
        <div className="bg-white border border-cream-200 p-6">
          ...
          <div className="space-y-3 mb-4">
            {(form.images ?? []).length === 0 && (
              <p className="font-body text-xs text-charcoal-700/40">No images added yet.</p>
            )}
            {(form.images ?? []).map((url, i) => (
              <div key={i} className="flex items-center gap-3 group">
                ...
                <button type="button" onClick={() => removeImage(i)}
                  className="p-1 text-charcoal-700/30 hover:text-red-500 transition-colors shrink-0">
                  <X size={14} />
                </button>
              </div>
            ))}
          ...
          <p className="font-body text-[11px] text-charcoal-700/30 mt-2">Paste an image URL and click Add. Press Enter to add quickly.</p>
        </div>
